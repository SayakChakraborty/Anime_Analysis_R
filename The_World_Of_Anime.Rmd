...
tidy_anime <- data.table::fread ("D:/OneDrive - University of Cincinnati/GitHub/SayakChakraborty/Anime Analysis in R/Anime_Analysis_R/anime_data/tidy_anime.csv", stringsAsFactors = FALSE, header = TRUE)
```
...
package_sum <- read_excel("D:/OneDrive - University of Cincinnati/GitHub/SayakChakraborty/Anime Analysis in R/Anime_Analysis_R/Package Summary.xlsx")
...
```{r Cleaned Dataset, echo=FALSE, warning=FALSE}
...
```{r displaying the summary variables table, echo=FALSE}
#Reading the variable summary excel File
var_sum <- read_excel("D:/OneDrive - University of Cincinnati/UC (LCB) - MSIS/Semester 1/BANA 7025 Data Wrangling with R/Final Project/variable_summary.xlsx")
...
<center>
```{r Analysing the number of anime trend, echo=FALSE,  warning=FALSE, results='markup'}
...
<center>
```{r Anime Type Popularity, echo=FALSE, warning=FALSE, results='markup'}
unique_anime %>% 
...
<center>
```{r Genre vs Mean Users Rating, echo=FALSE, warning=FALSE, results='markup' }
...
anime_final %>% 
  ...
  coord_flip() +
  theme_minimal() +
  geom_text(aes(label = round(mean_rating,digit = 2)), hjust = 2.0, color = "white", size = 3.5) +
...
```{r Number of Viewer Ratings per Genre, echo=FALSE, warning=FALSE, results='markup'}
...
```{r Displaying the most popular genres, echo=FALSE, warning=FALSE,}
anime_final %>%
  filter(!(is.na(genre))) %>% 
  group_by(genre) %>%
  summarize(total = sum(anime_id)) %>%
  ggplot(aes(x = reorder(genre, +total), y = total)) + 
  geom_bar(stat = "identity", fill = "blue", colour = "black", width = 0.8, position = position_dodge(width = 0.9)) + 
  coord_flip() + 
...
<center>
```{r Popularity Of Broadcasting Days, echo=FALSE, warning=FALSE, results='markup'}
anime_final %>% 
  ...
  geom_text(aes(label = round(mean_popularity_index,digit = 2)), hjust = 2.0, color = "red", size = 3.5) +
  theme_minimal() +
...
<center>
```{r Number of Users, echo=FALSE, warning=FALSE, results='markup'}
anime_final %>% 
  filter(!(is.na(broadcast_day))) %>% 
  filter(!(broadcast_day == "Unknown")) %>% 
  filter(!(broadcast_day == "Not")) %>% 
  ...
  geom_col(stat = "identity", color = "black") +
  coord_flip() +
  geom_text(aes(label = show_count), hjust = 2.0, color = "black", size = 3.5) +
...
<center>
```{r Popular Broadcasting Days, echo=FALSE, warning=FALSE, results='markup'}
unique_anime %>% 
  filter(!(is.na(broadcast_day))) %>% 
  filter(!(broadcast_day == "Unknown")) %>% 
  filter(!(broadcast_day == "Not")) %>% 
  ...
  scale_fill_brewer(palette = "OrRd") +
  theme_minimal() +
  geom_text(aes(label = round(mean_rating, digit = 2)), hjust = 2, color = "darkblue", size = 3.5) +
...
<center>
```{r Season wrt to Popularity Index  , echo=FALSE, warning=FALSE, results='markup'}
unique_anime %>% 
...
```{r Popular Premiered Season , echo=FALSE, warning=FALSE, results='markup'}
unique_anime %>% 
  filter(!(is.na(premiered_season))) %>% 
  filter(!(premiered_season == "NA")) %>%
  group_by(premiered_season) %>% 
...
<center>
```{r Most popular Anime Sources, echo=FALSE, warning=FALSE, results='markup'}
unique_anime %>%
...
<centre>
```{r Producers wrt to the Number of Animes, echo=FALSE, warning=FALSE, results='markup'}
unique_anime %>%
